# Portfolio Personnel - Joel MITONDO MOHINDO

Bienvenue sur le dépôt de mon portfolio professionnel. Ce projet est une vitrine de mon parcours atypique, combinant la **rigueur du Droit**, la **logique scientifique** et l'**innovation numérique**.

##  Aperçu du Projet
Ce portfolio a été conçu pour présenter mes compétences en développement web Full Stack tout en mettant en avant mes initiatives entrepreneuriales (**NMS Tech**, **SABES**).

**Lien du site en direct :** [Cliquez sur ce lien pour visionner le site](joelmitondo.github.io/Mon-Portfolio/)
---

## Stack Technique
Pour ce projet, j'ai privilégié des technologies modernes pour garantir performance, rapidité et un design haut de gamme :

* **Frontend :** HTML5, Tailwind CSS (Design System)
* **Design :** Glassmorphism & Bento Grid Layout
* **Animations :** Transitions fluides et Micro-interactions
* **SEO :** Optimisation Open Graph pour le partage social (WhatsApp, LinkedIn, etc.)

---

## Un Parcours Multidisciplinaire
Mon profil se distingue par une polyvalence rare qui nourrit ma vision de développeur :

1.  **Technologie :** Apprenant à la **Kadea Academy**, spécialisé en Dévéloppement Web.
2.  **Droit :** 3 ans d'études universitaires (Graduat) apportant une rigueur d'analyse et de rédaction.
3.  **Entrepreneuriat :** Fondateur de **SABES** (Agro-alimentaire Bio) et **NMS Tech** (Solutions Numériques).
4.  **Sciences :** Diplômé d'État en Biologie-Chimie.

---

## Structure du Projet
Le site est organisé en sections stratégiques :
* **Hero Section :** Introduction impactante.
* **Experiences :** La liste des compétences acquises.
* **Parcours :** Timeline interactive (du Primaire à Kadea).
* **Projets :** Galerie de réalisations tech.
* **À Propos et Témoignage:** Synthèse personnelle et valeurs, ainsi que la preuve sociale de collaborateurs et partenaires..
* **Contact :** Formulaire et accès direct vers mes réseaux.

---

## Aperçu Visuel
> ![Image de mon site](https://i.ibb.co/WYq8tNC/capture-portfolio.png)

---

## Me Contacter
Je suis toujours ouvert à de nouvelles opportunités de collaboration, que ce soit pour des projets web ambitieux ou des initiatives entrepreneuriales.

* **Localisation :** Kinshasa, RD Congo
* **Email :** Mitondomohindo@gmail.com
* **LinkedIn :** https://cd.linkedin.com/in/joel-mitondo-b23326385
* **WhatsApp :** https://we.me/243823310484

---
*Réalisé avec passion par Joel MITONDO — 2026*